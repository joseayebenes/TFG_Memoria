...
\begin{itemize}
	\item \textbf{Aplicación del coprocesador:} Es el programa ejecutandose en el dispositivo CC1350. Esta aplicación implementa una capa 802.15.4e/g MAC/PHY y proporciona una comunicación serie.
	\item \textbf{Kernel Linux:} El kernel Linux provee los controladores para la interfaz serie que está disponible en un puerto físico (por ejemplo, USB).
	\item \textbf{Aplicación TI 15.4-Stack:} Este módulo implementa la aplicación usando el protocolo 802.15.4e/g y la estructura del modelo \ac{MT}.
\end{itemize}

\subsection{Descripción del directorio del proyecto}


La figura \ref{fig:fig-oob-dir} muestra la estructura del directorio del proyecto del concentrador. A continuación se explica una descripción de alto nivel de cada carpeta:
\begin{figure}[h]
	\centering
...
\begin{description}
	...
	\item[docs] En esta carpeta se encuentra la documentación generada por Doxygen.
	...
	\item[run.sh] Ejecuta la aplicación.
	\item[PullBuild.sh] Descarga una nueva versión del código desde Github y compila el proyecto.
