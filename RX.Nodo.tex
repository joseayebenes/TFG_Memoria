...
El nodo está basado en la placa de desarrollo \textit{SimpleLink\TM CC1350 wireless microcontroller (MCU) LaunchPad\TM} que combina una interfaz radio \textit{Bluetooth\R low energy}  y  otra de \textit{Sub-1GHz} en un único chip.\\
...
\end{figure}
\subsubsection{ARM Cortex M0 (Núcleo radio)}
...
\begin{description}
	...
	\item[Funciones de control HW] Provee varias utilidades para usar LCD, temporizadores, botones y más.
	\item[Funciones de la aplicación:] Implementa funciones como guardado de datos, y provee una interfaz para gestionar pulsaciones botones o mostrar información esencial en un LCD.
...
En este proyecto se ha incluido un \ac{RTOS} que permita la separación en tareas de los distintos procesos. Para facilitar la compatibilidad entre los distintos módulos se ha utilizado el sistema operativo TI-RTOS de Texas Instruments.
...
Como se observa en la figura \ref{fig:fig-sensor-task-flow-chart} después de ejecutar la función \textit{Sensor\_init()} la tarea entra en un bucle donde se realizan los mismos procedimientos:

\begin{enumerate}
	\item Conectar o reconectar el nodo a la red.
	\item Enviar el mensaje periódico con los datos de sensores.
	\item Procesar los eventos LLC que gestionan la capa de enlace.
	\item Procesar los eventos específicos del sensor y enviar los mensajes de la web física en los intervalos definidos.
	\item Procesar los eventos de la pila ApiMac.
\end{enumerate}

\subsection{Funciones de gestión del Stack}
\input{RX.stack.tex}

\subsection{Funciones de acceso a la web física}
...
Los posibles mensajes entre el concentrador y el nodo, están definidos en el archivo \textit{smsgs.h}. En el Apéndice \ref{apd:estructurasMensajes} se puede encontrar un diagrama con las estructuras de estos mensajes.\\
