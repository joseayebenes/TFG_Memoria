% Preámbulo del documento.
%-----------------------------------------------------------------%
% Clase de documento: libro
\documentclass[12pt,a4paper]{book} % article, report, book.

% Preámbulo: paquetes, comandos, entornos, estilos y título de página.
\input{A1.Preambulo.paquetes.tex}
\input{A2.Preambulo.commandos.tex}
\input{A3.Preambulo.entornos.tex}
\input{A4.Preambulo.estilodepagina.tex}

%Comando para crear glosario (index en inglés)
\makeindex


% Document body.
%-------------------------------------------------------------------%
\begin{document}

% Formato de documento hasta el capítulo 1 (sin incluirlo)
\frontmatter

\renewcommand{\pfcauthorname}{José Antonio Yébenes Gálvez}
\renewcommand{\pfctitlename}{Plataforma inal\'{a}mbrica para la web física}
\renewcommand{\pfctitlenameenglish}{Wireless framework for physical web}
\renewcommand{\pfctutorname}{Ignacio Herrero y José Manuel Cano}
\renewcommand{\pfcanno}{2018}

% Portada.
\input{B1.Portada.tex}

% Hoja de calificación
\input{B2.Calificacion.tex}

% Resumen del proyecto (formulario)
\input{B3.ResumenTFG.tex}
\input{BX.ResumenInglesTFG.tex}

% Dedicatoria.
\input{B4.Dedicatoria.tex}

% Acrónimos
\input{B5.Acronimos.tex}

% Tabla de contenidos, figuras y tablas.
\input{B5.TableOfContents.tex}
\input{B5.TableOfFigures.tex}
\input{B5.TableOfTables.tex}

% Formato de documento durante los capítulos.
\mainmatter

% Prólogo.
%\input{B6.Prologo.tex}

% Parte I. Introducción
\part{Introducción}

\input{I1.Objetivo.tex}
%\input{I4.Estructura.tex}
%\input{I2.Especificaciones.tex}
\input{IX.EstadoArte.tex}



%\input{I3.Metodologia.tex}
%\input{E1.webFisica.tex}
%\input{E2.RedesInalambricas.tex}
%\input{E3.PlataformasComunicaciones.tex}
%\input{E4.LinuxEmbebido.tex}

\part{Desarrollo del proyecto}
\input{C1.Descripcion.tex}

\input{CX.RedInalambrica.tex}

%\input{CX.ti_stack.tex}

%\input{CX.Nodo.tex}

%\input{CX.Concentrador.tex}

\input{C3.Servidor.tex}

\part{Pruebas y funcionamiento}

\input{PX.EntornoReal.tex}
\input{PX.Consumo.tex}
\input{PX.Funcionamiento.tex}

\part{Conclusiones y lineas futuras}
\input{D0.Conclusiones.tex}
\input{D1.LineasFuturas.tex}
% Anexos
\part{Apéndices}

\appendix

\input{D1.AppendixA.tex}

%\input{D2.AppendixB.tex}

%\input{D3.AppendixC.tex}

% Formato de documento en la parte final.
\backmatter
%Hace que los capítulos y títulos nivel inferior no aparezcan numerados (lo que es ideal para conclusiones o notas finales).

% Bibliografía
\input{E1.Bibliografia.tex}

% Índice alfabético%
%\input{F1.Index.tex}

\end{document}